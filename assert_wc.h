...
{ \
    char buf[1024] = {'\0'}; \
    snprintf(buf, 1024, GET_1TH_ARG(__VA_ARGS__), GET_2TH_ARG(__VA_ARGS__), GET_3TH_ARG(__VA_ARGS__)); \
    qDebug() << buf; \
...
{ \
    char buf[1024] = {'\0'}; \
    snprintf(buf, 1024, GET_1TH_ARG(__VA_ARGS__), GET_2TH_ARG(__VA_ARGS__)); \
    qDebug() << buf; \
...
#define ASSERT_WITH_CODE_1(cond) \
        if(!(cond)) \
{ \
    qFatal("ASSERTION FAILED! Program terminated"); \
}
